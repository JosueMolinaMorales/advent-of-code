...
pub fn solve_day_seven() {
    ...
    println!("Day 7 part one: {}", part_one(&input));
    println!("Day 7 part two: {}", part_two(&input));
}

fn part_one(input: &str) -> i32 {
    let sizes = create_directory(input);
    let res = sizes
        .iter()
        .filter_map(|s| if s.1 < 100000 { Some(s.1) } else { None })
        .sum::<u32>();
...
fn part_two(input: &str) -> i32 {
    let mut sizes = create_directory(input);
    sizes.sort();
...
fn create_directory(input: &str) -> Vec<(String, u32)> {
