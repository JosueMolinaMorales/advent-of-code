...
fn parse_input_and_create_grid<'a>(part: Part) -> (Vec<Vec<&'a str>>, isize) {
    ...
    INPUT.lines().for_each(|lines| {
        ...
        for (i, pair) in pairs.iter().enumerate() {
            ...
            for j in 0..=dx.abs() {
                points.insert((prev_pair.0 + (j * dx.signum()), prev_pair.1), "#");
            }
            for j in 0..=dy.abs() {
                points.insert((prev_pair.0, prev_pair.1 + (j * dy.signum())), "#");
            }
            prev_pair = *pair;
        }
    });
    match part {
        Part::One => (
            create_grid(&points, min_col, max_row, max_col, false),
            min_col,
        ),
        Part::Two => {
            ...
            (
                create_grid(&points, min_col, max_row + 2, max_col, true),
                min_col,
            )
        }
    }
...
pub fn solve_day_14() {
    println!("Day 14 Part One: {}", solve_part_one());
    println!("Day 14 Part Two: {}", solve_part_two());
}
